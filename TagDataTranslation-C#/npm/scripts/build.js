...
const WASM_PROJECT = path.join(ROOT, "TagDataTranslation.Wasm", "TagDataTranslation.Wasm.csproj");
const DIST_WASM = path.join(__dirname, "..", "dist", "wasm");

console.log("Building TagDataTranslation.Wasm...");
execSync(
  `dotnet publish "${WASM_PROJECT}" -c Release -o "${path.join(ROOT, "TagDataTranslation.Wasm", "bin", "publish")}"`,
  { stdio: "inherit" }
);

// copy WASM output to dist/wasm/
const publishDir = path.join(ROOT, "TagDataTranslation.Wasm", "bin", "publish", "wwwroot", "_framework");
