...
VERSION=$(grep '<Version>' "$ROOT_DIR/src/TagDataTranslation/TagDataTranslation.csproj" | sed 's/.*<Version>\(.*\)<\/Version>.*/\1/')
echo "========================================"
...
dotnet test "$ROOT_DIR/test/TagDataTranslation.Tests/TagDataTranslation.Tests.csproj" -c Release --verbosity quiet
echo "Tests passed."
echo ""
...
publish_nuget() {
    ...
    dotnet pack "$ROOT_DIR/src/TagDataTranslation/TagDataTranslation.csproj" \
        -c Release \
...
publish_ios() {
    ...
    echo "  2. Update checksum in sdk/swift/Package.swift"
    echo "  3. pod trunk push sdk/swift/TagDataTranslation.podspec"
    echo ""
}
